...
def test_zip_with_prediction(depth):
    ...
    codecs.compress_image(
        fd, x, enums.Compression.zip_prediction, depth, 1)
    y = codecs.decompress_image(
        fd.getvalue(), enums.Compression.zip_prediction, (256, 256), depth, 1)
...
def test_zip(depth):
    ...
    fd = io.BytesIO()
    codecs.compress_image(
        fd, x, enums.Compression.zip, depth, 1)
    y = codecs.decompress_image(
        fd.getvalue(), enums.Compression.zip, (256, 256), depth, 1)
...
def test_rle(depth, version):
    ...
    fd = io.BytesIO()
    codecs.compress_image(
        fd, x, enums.Compression.rle, depth, version)
    y = codecs.decompress_image(
        fd.getvalue(), enums.Compression.rle, (256, 256), depth, version)
